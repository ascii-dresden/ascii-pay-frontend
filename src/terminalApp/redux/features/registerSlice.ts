...
function solveCashProblem(
  coinBox: CoinBoxState,
  noteBox: NoteBoxState
): [CoinBoxState, NoteBoxState] {
  ...
  while (currentTotal > targetTotal) {
    ...
    let boxHelper = [
      ...
      (it) =>
        currentTotalCapute - it.centValue >= targetTotal && it.currentCount > 0
    );
  ...
  reducers: {
    ...
    setRegisterMode: (state, action: PayloadAction<RegisterMode>) => {
      ...
      if (action.payload === RegisterMode.RESULT && state.previous === null) {
        let [nextCoinBox, nextNoteBox] = solveCashProblem(
          state.coinBox,
          state.noteBox
        );
        state.previous = {
