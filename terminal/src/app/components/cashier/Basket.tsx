    ...
    return (
      <div className="basket">
        ...
        <div className="basket-footer">
          ...
          <div className="basket-footer-pay" onClick={() => this.props.pay()}>
            Pay
          </div>
        </div>
      </div>
  ...
  render() {
    ...
    return (
      <div className="basket-entry">
        <div className="basket-entry-image" onClick={() => this.props.updateAmount(-1)}>
          <div>{image}</div>
        </div>
        <div className="basket-entry-name">{this.props.product.name}</div>
        <div className="basket-entry-price">
          {((this.props.amount * (this.props.product.current_price || 0)) / 100).toFixed(2)}€
        </div>
...
class BasketFreehand extends React.Component<BasketFreehandProps, BasketFreehandState> {
  ...
  render() {
    return (
      <div className="basket-entry">
        <div className="basket-entry-image" onClick={() => this.props.delete(this.props.index)}>
          <div>
            <div>
              <MdEuroSymbol />
            </div>
