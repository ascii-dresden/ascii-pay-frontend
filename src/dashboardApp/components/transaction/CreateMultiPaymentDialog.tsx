...
function checkIfAccountCanPay(
  account: AccountDto,
  total: CoinAmountDto
): boolean {
  let balance = subCoinAmount(account.balance, total);

  if (
    (balance.Cent ?? 0) < 0 &&
    (balance.Cent ?? 0) < (account.balance.Cent ?? 0)
  ) {
    return false;
  }
  if (
    (balance.BottleStamp ?? 0) < 0 &&
    (balance.BottleStamp ?? 0) < (account.balance.BottleStamp ?? 0)
  ) {
    return false;
  }
  if (
    (balance.CoffeeStamp ?? 0) < 0 &&
    (balance.CoffeeStamp ?? 0) < (account.balance.CoffeeStamp ?? 0)
  ) {
    return false;
  }
...
export const CreateMultiPaymentDialog = (props: {
  accounts: AccountDto[];
  open: boolean;
  ...
  const handleSubmit = async () => {
    ...
    if (items.length > 0 && props.accounts.length > 0) {
