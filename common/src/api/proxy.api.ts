...
export async function requestPaymentToken(amount: number) {
  return await requestProxyText(Method.POST, 'request-payment-token', {
    amount,
  });
}

export async function reauthenticate() {
  return await requestProxyText(Method.GET, 'reauthenticate-nfc');
}

export async function cancelTokenRequest() {
  return await requestProxyText(Method.GET, 'cancel');
}
